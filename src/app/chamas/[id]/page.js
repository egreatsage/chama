      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-6">
          ...
          <div className="py-6">
            ...
            {activeTab === 'members' && <MembersList members={members} chama={chama} onActionComplete={fetchData} />}
          </div>
        </div>
      </div>
    </ProtectedRoute>
...
function MembersList({ members, chama, onActionComplete }) {
    ...
    const handleInvite = async (e) => {
        ...
        try {
            ...
            setInviteEmail('');
            onActionComplete(); // Refresh the member list
        } catch (err) {
            toast.error(err.message, { id: toastId });
        } finally {
            setIsInviting(false);
    ...
    const handleRemove = async (memberId) => {
        if (window.confirm("Are you sure you want to remove this member from the Chama?")) {
                ...
                {members.map(member => (
                    <li key={member._id} className="py-4 flex justify-between items-center">
                        <div className="flex items-center">
                            <img className="h-10 w-10 rounded-full object-cover" src={member.userId?.photoUrl || `https://ui-avatars.com/api/?name=${member.userId?.firstName}+${member.userId?.lastName}`} alt="" />
                            <div className="ml-3">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            {chama.userRole === 'chairperson' && member.role !== 'chairperson' && (
                                <button onClick={() => handleRemove(member._id)} className="text-red-500 hover:text-red-700 text-xs font-semibold">
                                    Remove
                                </button>
                            )}
                        </div>
